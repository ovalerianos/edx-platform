...
class EffortEstimationTransformer(BlockStructureTransformer):
    ...
    def transform(self, usage_info, block_structure):
        ...
        # If we drop the experiment but keep the feature - we should leave estimates on all blocks, and just have the
        # UI show the estimate at the level they care about.
        bucket = EFFORT_ESTIMATION_LOCATION_FLAG.get_bucket(course_key=usage_info.course_key)
        if bucket == 0:
            return  # early exit, we don't need to do any of this work
        ...
        # estimates at different levels, per the experiment. If we make this real code and not an experiment, just
        # drop this whole section.
        for block_key in block_structure.post_order_traversal():
            category = block_structure.get_xblock_field(block_key, 'category')
            if (bucket == 1 and category != 'chapter') or (bucket == 2 and category != 'sequential'):
